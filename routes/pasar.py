...
def read_pasars(skip: int = 0, limit: int = 100):
    ...
    return str(pages.pasars_page(pasars))
...
def read_pasar(pasar_id: int):
    ...
    return str(pages.pasar_detail(pasar, lock=True))
...
def edit_pasar(req: Request, pasar_id: int):
    ...
    if req.headers.get('HX-Request'):
        return str(pages.pasar_form(pasar, rph, lock))
    else:
        return str(pages.pasar_detail(pasar, rph, lock))
...
async def update_pasar(
    ...
    rph = Crud(models.Rph, next(get_db())).get()
    return str(pages.pasar_form(pasar, rph, lock=True))
...
def remove_pasar(pasar_id: int):
    ...
    return str(pages.pasars_table(pasars))
