...
def user_form(user=None, role=None, actors=None, lock: bool = False) -> Element:
    if lock:
        ...
        role = inlabel(
            "Role", "text", "role", (role_list[user.role.role] if user else ""), lock
        )
        actor = inlabel(
            "Acting As",
            "text",
        ...
        form_btn = update_btn("/users/", user.username) if user else submit_btn

    return form(
        "#form", action="/users/", method="post", enctype="multipart/form-data"
    )[
        (None if user else inlabel("Username", "text", "username", "", lock)),
        inlabel("E-mail", "email", "email", (user.email if user else ""), lock),
        (
            a(
                role="button",
        ...
        inlabel("Phone", "text", "phone", (user.phone if user else ""), lock),
        role,
        actor,
        form_btn,
    ]
    ...
    return dialog("#ubah_password", open=True)[
        form(
            ...
            article[
                ...
                inlabel("Password Lama", "password", "password"),
                inlabel("Password Baru", "password", "password_new"),
                inlabel(
