...
def pasar_form(pasar, lock: bool = False) -> Element:
    ...
    return form("#form", action="/pasar", method="post", enctype="multipart/form-data")[
        inlabel("Nama", "text", "name", (pasar.name if pasar else ""), lock),
        inlabel("Alamat", "text", "alamat", (pasar.alamat if pasar else ""), lock),
        form_btn,
    ]
